...
public class hardGame extends Game {
    ...
    public hardGame(Context context, Handler handler) {
        ...
        enemyMaxNumber = 10;
        System.out.println("hard mode, max quantity of enemies is:" + this.enemyMaxNumber);
        level = 3;
    }
...
//        随机产生MobEnemy或者EliteEnemy,当分数是200的整数倍时，产生BossEnemy
        ...
        if (score % 200 <= 50 & score % 200 >= 0& score != 0 & score >= 200 & !isBossExist) {
            enemyFactory = new BossFactory();
            BossEnemy boss = (BossEnemy) enemyFactory.createEnemy();
            BOSS_HP = (int) (difficulty * boss.getMaxHp());
            boss.setMaxHp(BOSS_HP);
